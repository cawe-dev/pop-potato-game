defmodule PopPotatoGame.LobbyTest do
  ...
  describe "rooms" do
    ...
    import PopPotatoGame.AccountsFixtures, only: [user_scope_fixture: 0]
    import PopPotatoGame.LobbyFixtures
    ...
    test "list_rooms/1 returns all scoped rooms" do
      ...
      room = scoped_room_fixture(scope)
      other_room = scoped_room_fixture(other_scope)
      assert Lobby.list_rooms(scope) == [room]
      assert Lobby.list_rooms(other_scope) == [other_room]
    end
    ...
    test "get_room!/2 returns the room with given id" do
      ...
      room = scoped_room_fixture(scope)
      other_scope = user_scope_fixture()
      assert Lobby.get_room!(scope, room.id) == room
      assert_raise Ecto.NoResultsError, fn -> Lobby.get_room!(other_scope, room.id) end
    end
    ...
    test "create_room/2 with valid data creates a room" do
      ...
      assert {:ok, %Room{} = room} = Lobby.create_room(scope, valid_attrs)
      assert room.code == "some code"
      assert room.status == "some status"
      assert room.type == "some type"
      assert room.password == "some password"
    ...
    test "create_room/2 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      assert {:error, %Ecto.Changeset{}} = Lobby.create_room(scope, @invalid_attrs)
    end
    ...
    test "update_room/3 with valid data updates the room" do
      ...
      assert {:ok, %Room{} = room} = Lobby.update_room(scope, room, update_attrs)
      assert room.code == "some updated code"
      assert room.status == "some updated status"
      assert room.type == "some updated type"
      assert room.password == "some updated password"
    ...
    test "update_room/3 with invalid scope raises" do
      ...
      assert_raise MatchError, fn ->
        Lobby.update_room(other_scope, room, %{})
      end
    end
    ...
    test "update_room/3 with invalid data returns error changeset" do
      scope = user_scope_fixture()
      room = scoped_room_fixture(scope)
      assert {:error, %Ecto.Changeset{}} = Lobby.update_room(scope, room, @invalid_attrs)
      assert room == Lobby.get_room!(scope, room.id)
    end

    test "delete_room/2 deletes the room" do
      scope = user_scope_fixture()
      room = scoped_room_fixture(scope)
      assert {:ok, %Room{}} = Lobby.delete_room(scope, room)
      assert_raise Ecto.NoResultsError, fn -> Lobby.get_room!(scope, room.id) end
    end
    ...
    test "delete_room/2 with invalid scope raises" do
      ...
      other_scope = user_scope_fixture()
      room = scoped_room_fixture(scope)
      assert_raise MatchError, fn -> Lobby.delete_room(other_scope, room) end
    end

    test "change_room/2 returns a room changeset" do
      scope = user_scope_fixture()
      room = scoped_room_fixture(scope)
      assert %Ecto.Changeset{} = Lobby.change_room(scope, room)
    end
  end
end
