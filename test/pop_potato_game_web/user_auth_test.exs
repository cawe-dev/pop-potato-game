defmodule PopPotatoGameWeb.UserAuthTest do
  use PopPotatoGameWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias PopPotatoGame.Accounts
  alias PopPotatoGame.Accounts.Scope
  alias PopPotatoGameWeb.UserAuth

  import PopPotatoGame.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PopPotatoGameWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PopPotatoGameWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PopPotatoGameWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
